...
class PersonalInformation extends Component {
    ...
    verifyNumber = () => {
        ...
        if (
            (numberString.length === 9 && numberString.charAt(0) !== "0" && (numberString.charAt(0) === "4" || numberString.charAt(0) === "5")) ||
            (numberString.length === 10 && numberString.charAt(0) === "0" && (numberString.charAt(1) === "4" || numberString.charAt(1) === "5"))
        ) {
            return true;
        }
    ...
    verifyLength = () => {
        for (let i = 0; i < this.state.user_data.length; i++) {
            if (i !== 5 && this.state.user_data[i].value.length === 0) {
                // Medicare number is optional so no need to check its length
    ...
    render() {
        return (
            <div className="user-info-container">
                    ...
                    {this.state.userRole === "ROLE_PROVIDER" && (
                        <>
                            {this.state.user_data.map((item, index) => (
                                <div className="user-info-content" key={item.key}>
                                    <label>{item.key}:</label>
                                    <input type={item.type} onChange={(event) => this.saveData(index, event)} placeholder={item.key} value={item.value} />
                                </div>
                            ))}
                        </>
                    )}
                ...
                <p>
                    <b>Note:</b> Click 'Save Changes' to save your edits.
                </p>
                <button className="save-changes-btn" onClick={this.saveChanges}>
                    Save Changes
                </button>
            </div>
        );
